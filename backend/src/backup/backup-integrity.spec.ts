...
describe('Backup Integrity (M6)', () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                },
                {
                    ...
                    useValue: {
                        createEncryptStream: jest.fn(() => new (require('stream').PassThrough)()),
                        createDecryptStream: jest.fn(() => new (require('stream').PassThrough)()),
                    },
                },
            ],
        }).compile();
    ...
    async function createFakeBackup(content: string, hasHmac: boolean, key: string = secret) {
    ...
    it('should allow restore for a valid signed backup', async () => {
        ...
        jest.spyOn(service as any, 'decryptAndExtract').mockImplementation(async (input: string, outputDir: string) => {
            if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });
            fs.writeFileSync(path.join(outputDir, 'manifest.json'), JSON.stringify({ appVersion: '1.0.0' }));
    ...
    it('should block restore if verification key is different', async () => {
        const filename = await createFakeBackup('content signed with other key', true, 'other-secret-key-32-chars-long-!!!');
    ...
    it('should block restore even with force=true if signature is invalid', async () => {
        ...
        await expect(service.restoreSystemSnapshot(filename, { force: true })).rejects.toThrow(
            /Security Error: Invalid HMAC signature/
        );
    });
});

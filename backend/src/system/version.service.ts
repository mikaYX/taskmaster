...
export class VersionService {
  ...
  private async fetchAndCache(): Promise<VersionStatusDto> {
    ...
    const offlineMode = this.config.get<boolean>('OFFLINE_MODE', false);
    const lookup: VersionLookupResult = offlineMode
      ? {
          latestVersion: null,
          releaseUrl: null,
          sourceStatus: 'degraded',
          error: 'Update checks disabled (OFFLINE_MODE)',
        }
      : await this.lookupLatestVersion(source);

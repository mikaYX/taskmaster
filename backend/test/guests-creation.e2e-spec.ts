...
describe('Guests Creation and Flow (e2e)', () => {
    ...
    it('POST /users should FAIL when trying to create role GUEST', () => {
        return request(app.getHttpServer())
            ...
            .expect(409)
            .expect(res => {
                expect(res.body.message).toContain('Guest users cannot be created via the standard user endpoint');
            });
    });
    ...
    it('POST /guests/site/:siteId should SUCCEED for admin', async () => {
        ...
        await request(app.getHttpServer())
            ...
            .expect(409)
            .expect(res => {
                expect(res.body.message).toContain('active guest already exists for site');
            });
    });

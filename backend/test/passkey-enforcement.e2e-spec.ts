...
describe('Passkeys Hard Enforcement (E2E)', () => {
    ...
    afterEach(async () => {
        ...
        for (const role of roles) {
            await prisma.client.config.upsert({
                ...
                update: { value: 'false' },
                create: { key: `security.enforcement.passkeys.${role}`, value: 'false' }
            });
            await prisma.client.config.upsert({
    ...
    it('blocks access to protected routes when policy is REQUIRED and NO passkey (403)', async () => {
        // Enable passkeys
        await prisma.client.config.update({ where: { key: 'auth.passkeys.enabled' }, data: { value: 'true' } });
        // Set policy to required for USER
        await prisma.client.config.update({ where: { key: 'security.enforcement.passkeys.USER' }, data: { value: 'true' } });
        (settingsService as any).cache.clear();
        await redis.flushdb();
    ...
    it('allows access to whitelisted endpoints (exempt routes) even if policy is REQUIRED and NO passkey', async () => {
        ...
        await prisma.client.config.upsert({ where: { key: 'auth.passkeys.enabled' }, update: { value: 'true' }, create: { key: 'auth.passkeys.enabled', value: 'true' } });
        await prisma.client.config.upsert({ where: { key: 'security.enforcement.passkeys.user' }, update: { value: 'true' }, create: { key: 'security.enforcement.passkeys.user', value: 'true' } });
        (settingsService as any).cache.clear();
        await redis.flushdb();

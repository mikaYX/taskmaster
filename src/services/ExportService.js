...
function toCsv(rows) {
    ...
    const header = ["task_id", "periodicity", "description", "procedure_url", "start_ts", "end_ts", "status", "comment"];
...
async function executeExport(cfg, { from, to, status = "", search = "", skipMail = false }) {
    ...
    if (!skipMail && cfg.mail_enabled && cfg.mail_export_enabled && nodemailer && generatedFiles.length > 0) {
        ...
        if (filesToSend.length > 0 && cfg.smtp_host && cfg.smtp_port && cfg.mail_from && cfg.mail_to) {
            ...
            await transporter.sendMail({
                from: cfg.mail_from,
                to: cfg.mail_to,
                subject: `Tasmaster export ${from} -> ${to}`,
                text: `Export generated. Rows: ${rows.length}`,
